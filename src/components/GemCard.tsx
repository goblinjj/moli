  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all duration-200 select-none">
      ...
      <div className={`card-expand overflow-hidden ${expanded ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="border-t border-gray-100 py-3 text-sm">
          <div className="overflow-x-auto px-4">
            <table className="text-xs min-w-full">
              <thead>
                <tr className="text-gray-400 border-b border-gray-100">
                  <th className="text-left py-1.5 font-medium pr-4 whitespace-nowrap">等級</th>
                  {targets.map((t) => (
                    <th key={t} className="text-left py-1.5 font-medium pr-4 whitespace-nowrap">
                      {targetLabels[t] || t}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {gem.levels.map((lv) => (
                  <tr key={lv.level} className="border-t border-gray-50 hover:bg-gray-50/50">
                    <td className="py-1.5 text-gray-500 font-mono pr-4 whitespace-nowrap">
                    ...
                    {targets.map((t) => {
                      ...
                      return (
                        <td key={t} className="py-1.5 text-gray-600 pr-4">
                          {effect?.description || "-"}
                        </td>
                      );
                    })}
